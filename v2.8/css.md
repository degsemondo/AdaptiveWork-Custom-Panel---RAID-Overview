...
.act-status:hover { background: rgba(139, 58, 98, 0.10); border-color: #8B3A62; }
...
/* Calculated Risk rating (C_RiskRating) RAG scale: Green / Yellow / Amber / Red */
.heatmap-rag-green  { background: #2e7d32; }
.heatmap-rag-yellow { background: #c9a000; }
.heatmap-rag-amber  { background: #e67e00; }
.heatmap-rag-red    { background: #c62828; }
...
/* Inline auto-commit: brief "Saving…" state in a cell */
.cell-saving {
  font-size: 11px;
  font-style: italic;
  color: #5f6368;
...
/* ── Row-level action (e.g. Create Issue on a Risk) ── */
.row-action-cell {
  text-align: center;
  white-space: nowrap;
}
...
.create-issue { color: #8B3A62; }
.create-issue:hover:not(:disabled) {
  background: rgba(139, 58, 98, 0.10);
  border-color: #8B3A62;
}
