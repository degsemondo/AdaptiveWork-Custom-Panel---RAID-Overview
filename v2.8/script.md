...
function renderIssues(issues, actionMap) {
  ...
  tbody.innerHTML = issues.map(function (issue) {
    ...
    return '<tr class="data-row" data-status="' + esc(issue.status) + '">' +
      ...
      editCell('C_ImpactDate',     issue.rawId, issue.impactDate || '',  '<span class="action-due ' + dueCls(issue.impactDate) + '">' + fmtDate(issue.impactDate) + '</span>') +
      '</tr>' +
...
function commitEdit(cell) {
  ...
  updateField(id, field, newValue)
    .then(function () {
      ...
      if (field === 'C_Impact' || field === 'C_Likelihood' || field === 'C_IssueImpact') { location.reload(); return; }
