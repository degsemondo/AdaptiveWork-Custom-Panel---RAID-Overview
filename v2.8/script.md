...
/* Resolve a Risk Status label to its real "/Type/Value" path. Prefer a value
   already present in this environment (metadata/data); otherwise derive it from
   the State prefix the environment itself uses — so "Closed" becomes
   /CaseState/Closed when the tenant's states live under /CaseState/. */
function statePrefix() {
  var lists = [PICK_META.State, PICK.State];
...
function stateRawForLabel(label) {
  return pickRawByLabel('State', [label]) || (statePrefix() + label);
}
/* Option list for a Risk's inline Status dropdown — RISK_STATE_OPTIONS only,
   plus the row's current value if it isn't one of them (so an existing Draft /
   Realised risk still shows and can be moved to Open / Closed). */
function riskStateOptionList(current) {
  var out = [], seen = {};
  RISK_STATE_OPTIONS.forEach(function (lbl) {
    var raw = stateRawForLabel(lbl);
...
function riskStateOptionsHtml(current, includeBlank) {
  var html = includeBlank ? '<option value="">—</option>' : '';
  return html + riskStateOptionList(current).map(function (o) {
