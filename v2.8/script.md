...
function renderRisks(risks, actionMap) {
  ...
  tbody.innerHTML = risks.map(function (r) {
    ...
    return '<tr class="data-row" data-status="' + esc(r.status) + '">' +
      ...
      '<td style="white-space:nowrap">' + idLink(r.rawId, r.sysId) + actionBadge(actions.length) + descIcon(r) + '</td>' +
...
function renderIssues(issues, actionMap) {
  ...
  tbody.innerHTML = issues.map(function (issue) {
    ...
    return '<tr class="data-row" data-status="' + esc(issue.status) + '">' +
      ...
      '<td style="white-space:nowrap">' + idLink(issue.rawId, issue.sysId) + actionBadge(actions.length) + descIcon(issue) + '</td>' +
...
function renderRequests(requests, actionMap) {
  ...
  tbody.innerHTML = requests.map(function (req) {
    ...
    return '<tr class="data-row" data-status="' + esc(req.status) + '">' +
      ...
      '<td style="white-space:nowrap">' + idLink(req.rawId, req.sysId) + actionBadge(actions.length) + descIcon(req) + '</td>' +
...
function saveNewRow(row) {
  ...
  /* New Risks start with State = Draft. */
  if (entityType === 'Risk' && !fields.State) {
    var draftState = pickRawByLabel('State', ['Draft']);
...
var SORT_COLS = {
  risks:  { 2: { f: 'name', t: 's' }, 5: { f: 'riskRating', t: 'score' }, 6: { f: 'status', t: 's' }, 7: { f: 'owner', t: 's' }, 9: { f: 'impactDate', t: 'd' } },
  issues: { 2: { f: 'name', t: 's' }, 4: { f: 'score',      t: 'score' }, 5: { f: 'status', t: 's' }, 6: { f: 'owner', t: 's' }, 8: { f: 'impactDate', t: 'd' } }
};
