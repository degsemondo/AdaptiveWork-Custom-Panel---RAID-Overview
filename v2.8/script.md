...
/* Selectable states when editing an ActionItem; new actions default to Open.
   Same treatment as risk Status — only these are offered, current value kept. */
var ACTION_STATE_OPTIONS = ['Open', 'Closed'];
...
/* ActionItem status dropdown (Open / Closed) + the raw path for a given label. */
function actionStateOptionsHtml(current, includeBlank) {
  return restrictedOptionsHtml('ActionItemState', ACTION_STATE_OPTIONS, '/ActionItemState/', current, includeBlank);
}
function actionStateRaw(label) {
  return rawForLabel('ActionItemState', label, '/ActionItemState/');
}
...
function actionBadge(n) {
  ...
  return ' <span class="action-badge" title="' + n + ' associated action' + (n === 1 ? '' : 's') + '"' +
    ' style="display:inline-block;min-width:16px;height:16px;line-height:16px;padding:0 5px;' +
    'border-radius:8px;background:#8B3A62;color:#fff;font-size:10px;font-weight:600;' +
...
function renderRisks(risks, actionMap) {
  ...
  tbody.innerHTML = risks.map(function (r) {
    ...
    return '<tr class="data-row" data-status="' + esc(r.status) + '">' +
      ...
      '<td>' + idLink(r.rawId, r.sysId) + actionBadge(actions.length) + '</td>' +
...
function renderIssues(issues, actionMap) {
  ...
  tbody.innerHTML = issues.map(function (issue) {
    ...
    return '<tr class="data-row" data-status="' + esc(issue.status) + '">' +
      ...
      '<td>' + idLink(issue.rawId, issue.sysId) + actionBadge(actions.length) + '</td>' +
...
function renderRequests(requests, actionMap) {
  ...
  tbody.innerHTML = requests.map(function (req) {
    ...
    return '<tr class="data-row" data-status="' + esc(req.status) + '">' +
      ...
      '<td>' + idLink(req.rawId, req.sysId) + actionBadge(actions.length) + '</td>' +
...
function completeRiskActions(c, riskId) {
  ...
  /* Actions now use Open / Closed — mark them Closed (fall back to legacy done
     labels if that's what the tenant uses). */
  var doneRaw = pickRawByLabel('ActionItemState', ['Closed', 'Completed', 'Complete', 'Done']) || actionStateRaw('Closed');
