...
function riskRatingHeatMap(val) {
  ...
  /* Calculated Risk rating (C_RiskRating) is a RAG word — Green / Yellow /
     Amber / Red — derived from Impact x Probability. Match these first. */
  if (s.indexOf('green') > -1)  return 'heatmap-rag-green';
  if (s.indexOf('amber') > -1)  return 'heatmap-rag-amber';
  if (s.indexOf('yellow') > -1) return 'heatmap-rag-yellow';
  if (s.indexOf('red') > -1)    return 'heatmap-rag-red';
...
function saveAction(el) {
  ...
  if (!name) { showToast('Action name is required.'); if (nameEl) nameEl.focus(); return; }
  ...
  var c;
  try { c = getContext(); } catch (e) { showToast('Cannot save action: ' + e.message, { error: true }); return; }
  ...
  p.then(function () { return loadAndRender(); })
   .catch(function (err) {
     showToast('Failed to save action: ' + err.message, { error: true });
...
function deleteAction(el) {
  ...
  showConfirm({
    ...
    onConfirm: function () {
      var c;
      try { c = getContext(); } catch (e) { showToast('Cannot delete action: ' + e.message, { error: true }); return; }
      deleteObject(c.base, c.sid, actionId)
        .then(function () { return loadAndRender(); })
        .catch(function (err) { showToast('Failed to delete action: ' + err.message, { error: true }); });
    }
  });
}
...
function openActionStatus(el) {
  if (!el) return;
  var actionId = el.getAttribute('data-action-id');
  if (!actionId) { showToast('Save the action first, then add a status update.'); return; }
...
/* In-panel TOAST — the non-jarring replacement for window.alert(). A small card
   sits bottom-centre, never dims the page, never blocks, and auto-dismisses
   (click it to dismiss early). Newlines in the message are preserved.
   opts = { error?: bool, duration?: ms }. */
function showToast(message, opts) {
  opts = opts || {};
  var t = document.createElement('div');
  t.setAttribute('role', 'status');
  t.style.cssText = 'position:fixed;z-index:2100;left:50%;bottom:24px;transform:translateX(-50%);' +
    'max-width:560px;padding:10px 16px;border-radius:8px;box-shadow:0 6px 22px rgba(0,0,0,0.22);' +
    'font:13px/1.45 system-ui,-apple-system,"Segoe UI",Roboto,sans-serif;white-space:pre-line;' +
    'cursor:pointer;color:#fff;background:' + (opts.error ? '#A32D2D' : '#202124') + ';';
  t.textContent = String(message == null ? '' : message);
  function dismiss() { if (t.parentNode) t.parentNode.removeChild(t); }
  t.addEventListener('click', dismiss);
  document.body.appendChild(t);
  setTimeout(dismiss, opts.duration || (opts.error ? 7000 : 4500));
}
...
function openDescription(rawId) {
  ...
  showDescriptionDialog({
    ...
    onSave: function (text) {
      ...
      var c;
      try { c = getContext(); } catch (e) { showToast('Cannot save description: ' + e.message, { error: true }); return; }
      updateObject(c.base, c.sid, rec.rawId, { Description: text })
        .then(function () { rec.description = text; return loadAndRender(); })
        .catch(function (err) { showToast('Failed to save description: ' + err.message, { error: true }); });
    }
  });
}
...
function createEntity(entityType, fields, label) {
  var c;
  try { c = getContext(); } catch (e) { showToast('Cannot create ' + label + ': ' + e.message, { error: true }); return Promise.reject(e); }
  ...
  return createObject(c.base, c.sid, entityType, fields)
    ...
    .catch(function (err) { showToast('Failed to create ' + label + ': ' + err.message, { error: true }); throw err; });
}
...
function scoreVal(v) {
  ...
  var rank = { 'heatmap-rag-green': 1, 'heatmap-rag-yellow': 2, 'heatmap-rag-amber': 3, 'heatmap-rag-red': 4,
               'heatmap-very-low': 1, 'heatmap-low': 2, 'heatmap-medium': 3, 'heatmap-high': 4, 'heatmap-very-high': 5, 'heatmap-critical': 6 };
...
/* ---------- 17b. Export to Excel (.xlsx) ----------
   Builds a GENUINE Office Open XML workbook (.xlsx) entirely in-browser — no
...
var EXPORT_SHEETS = {
  risks: {
    ...
    dropdowns: [
      ...
      { hdr: 'Probability',     list: 'riskProbability', recordOnly: true },
      { hdr: 'Impact',          list: 'riskImpact',      recordOnly: true },
      { hdr: 'Reporting Level', list: 'riskReporting',   recordOnly: true },
      { hdr: 'Status',          list: 'riskStatus', actionList: 'actionStatus' },
      { hdr: 'Due Date',        date: true },
      { hdr: 'Next Review Date', date: true, recordOnly: true }
    ],
    record: function (r) {
  ...
  issues: {
    ...
    dropdowns: [
      ...
      { hdr: 'Impact',          list: 'issueImpact',    recordOnly: true },
      { hdr: 'Reporting Level', list: 'issueReporting', recordOnly: true },
...
/* <dataValidations> for one sheet — one rule per configured column:
   - list dropdowns (`list` / `inline`), covering the data rows plus a generous
     run of blank rows beneath so rows the user adds still get them;
   - `recordOnly` rules skip the exported ACTION rows entirely (no arrow there)
     and, on the blank tail rows, blank the list if Type is set to "Action";
   - Status switches list by the row's Type via IF() — record rows offer case
     states, action rows offer Open / Closed / Stuck;
   - `date: true` columns get Excel date validation (any real date), so typed
     non-dates are rejected in the sheet before they reach the import. */
var DROPDOWN_EXTRA_ROWS = 500;
/* Compress a sorted list of 1-based row numbers into "L2:L4 L7 L9:L12" refs. */
function rowRanges(L, rows) {
  var out = [], i = 0;
  while (i < rows.length) {
    var start = rows[i], end = start;
    while (i + 1 < rows.length && rows[i + 1] === end + 1) { i++; end = rows[i]; }
    out.push(start === end ? L + start : L + start + ':' + L + end);
    i++;
  }
  return out;
}
function xlsxValidations(header, dropdowns, refs, kinds) {
  if (!dropdowns || !dropdowns.length) return '';
  var typeCol = header.indexOf('Type'), out = [];
  var lastRow = kinds.length + 1;
  var recordRows = [];
  kinds.forEach(function (k, i) { if (k !== 'action') recordRows.push(i + 2); });
  var tailFrom = lastRow + 1, tailTo = lastRow + DROPDOWN_EXTRA_ROWS;
  var typeRef = typeCol > -1 ? '$' + colLetter(typeCol) + '2' : '';

  dropdowns.forEach(function (d) {
    var ci = header.indexOf(d.hdr);
    if (ci < 0) return;
    var L = colLetter(ci);
    /* Cells the rule covers: every data row, or record rows only, plus the
       blank tail beneath the data. */
    var sqref = d.recordOnly
      ? rowRanges(L, recordRows).concat([L + tailFrom + ':' + L + tailTo]).join(' ')
      : L + '2:' + L + tailTo;

    if (d.date) {
      out.push('<dataValidation type="date" operator="greaterThan" allowBlank="1" showInputMessage="1" showErrorMessage="1" ' +
        'errorStyle="stop" errorTitle="Not a date" error="Please enter a valid date." sqref="' + sqref + '">' +
    ...
    if (d.inline && d.inline.length) {
      ...
      if (main && alt && typeRef) formula = 'IF(' + typeRef + '="Action",' + alt + ',' + main + ')';
      else if (main && d.recordOnly && typeRef) formula = 'IF(' + typeRef + '="Action","",' + main + ')';
...
/* JS Date -> Excel day serial (days since 1899-12-30), from the LOCAL calendar
   date so a due date never slips a day across time zones. */
...
/* cellXfs index for a Score cell (mirrors the on-screen heat map). */
var XLSX_HEAT_XF = {
  'heatmap-rag-green': 6, 'heatmap-rag-yellow': 7, 'heatmap-rag-amber': 8, 'heatmap-rag-red': 9,
  'heatmap-very-low': 10, 'heatmap-low': 11, 'heatmap-medium': 12, 'heatmap-high': 13,
  'heatmap-very-high': 14, 'heatmap-critical': 15
};
function exportHeatXf(value) {
  return XLSX_HEAT_XF[riskRatingHeatMap(value)] || 2;   /* neutral/blank -> plain record cell */
}
/* styles.xml — cellXfs: 0 default, 1 header, 2 record, 3 action, 4 record date,
   5 action date, 6-15 heat fills (see XLSX_HEAT_XF). */
function xlsxStyles() {
  ...
  var xfs = '<xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/>' +
    ...
    heatFills.map(function (c, i) {
      return '<xf numFmtId="0" fontId="1" fillId="' + (4 + i) + '" borderId="1" xfId="0" applyFont="1" applyFill="1" applyBorder="1" applyAlignment="1">' +
  ...
  return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' +
    ...
    '<borders count="2">' +
      ...
      '<border><left style="thin"><color rgb="FFE0E0E0"/></left><right style="thin"><color rgb="FFE0E0E0"/></right>' +
        '<top style="thin"><color rgb="FFE0E0E0"/></top><bottom style="thin"><color rgb="FFE0E0E0"/></bottom><diagonal/></border>' +
    '</borders>' +
    '<cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs>' +
    '<cellXfs count="' + (6 + heatFills.length) + '">' + xfs + '</cellXfs>' +
...
function xlsxSheet(header, built, heatCol, dropdowns, listRefs) {
  ...
  built.rows.forEach(function (row, idx) {
    ...
    rows.push('<row r="' + rn + '"' + (kind === 'action' ? ' outlineLevel="1"' : '') + '>' + row.map(function (val, c) {
      ...
      else if (heatCol != null && c === heatCol) xf = exportHeatXf(val);
      else xf = 2;
      return cell(colLetter(c) + rn, val, xf);
    }).join('') + '</row>');
  });
  return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' +
    '<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" ' +
    ...
    xlsxValidations(header, dropdowns, listRefs || {}, built.kinds) +
    '</worksheet>';
}

/* Minimal ZIP writer (STORED entries + CRC-32) — enough to package an .xlsx. */
...
function zipStore(files) {   /* files = [{ name, text }] -> Blob */
  var enc = new TextEncoder(), parts = [], central = [], offset = 0;
  function u16(n) { return [n & 0xFF, (n >>> 8) & 0xFF]; }
  function u32(n) { return [n & 0xFF, (n >>> 8) & 0xFF, (n >>> 16) & 0xFF, (n >>> 24) & 0xFF]; }
  /* Fixed DOS timestamp (2026-01-01) — Excel ignores it, but all-zero is invalid. */
  var dosTime = u16(0), dosDate = u16(((2026 - 1980) << 9) | (1 << 5) | 1);
  files.forEach(function (f) {
    var name = enc.encode(f.name), data = enc.encode(f.text), crc = crc32(data);
    var local = [].concat(u32(0x04034b50), u16(20), u16(0), u16(0), dosTime, dosDate, u32(crc),
      u32(data.length), u32(data.length), u16(name.length), u16(0));
    parts.push(new Uint8Array(local), name, data);
    central.push([[].concat(u32(0x02014b50), u16(20), u16(20), u16(0), u16(0), dosTime, dosDate, u32(crc),
      u32(data.length), u32(data.length), u16(name.length), u16(0), u16(0), u16(0), u16(0), u32(0), u32(offset)), name]);
    offset += local.length + name.length + data.length;
  });
  var cdStart = offset, cdSize = 0;
  central.forEach(function (c) { var h = new Uint8Array(c[0]); parts.push(h, c[1]); cdSize += h.length + c[1].length; });
...
function buildXlsx(sheets, listsSheet) {
  ...
  files.unshift(
    ...
    { name: 'xl/_rels/workbook.xml.rels', text:
      ...
      '</Relationships>' },
    { name: 'xl/styles.xml', text: xlsxStyles() }
  );
...
function exportToExcel() {
  ...
  a.href = url;
  a.download = 'RAID_Export_' + new Date().toISOString().slice(0, 10) + '.xlsx';
...
function importConvert(field, kind, display) {
  ...
  if (kind === 'date') {
    /* A native .xlsx date cell arrives as an Excel day serial (days since
       1899-12-30), e.g. "46229". Convert those; anything else parses as text. */
    if (/^\d{5}(\.\d+)?$/.test(display)) {
      var serial = Math.round(parseFloat(display));
      return Promise.resolve(new Date(Date.UTC(1899, 11, 30) + serial * 86400000).toISOString().slice(0, 10));
    }
...
/* ---------- .xlsx reading ----------
   Minimal ZIP reader for a native .xlsx: walks the central directory, extracts
   each part (STORED as-is; DEFLATE via the browser's native DecompressionStream
   — Excel-saved files are always deflated), then parses workbook / rels /
   sharedStrings / sheet XML into the same { sheetName: [[cell, ...], ...] }
   shape the legacy SpreadsheetML parser produces. Numbers (incl. Excel date
   serials) arrive as their text; importConvert turns serials into dates. */
function zipEntries(buf) {
  var b = new Uint8Array(buf), dv = new DataView(buf), i = b.length - 22, eocd = -1;
  for (; i >= 0 && i >= b.length - 22 - 65535; i--) { if (dv.getUint32(i, true) === 0x06054b50) { eocd = i; break; } }
  ...
  for (var n = 0; n < count; n++) {
    ...
    var name = dec.decode(b.subarray(p + 46, p + 46 + nlen));
    var lnlen = dv.getUint16(loff + 26, true), lxlen = dv.getUint16(loff + 28, true);
    var start = loff + 30 + lnlen + lxlen;
    out[name] = { method: method, data: b.subarray(start, start + csize), size: usize };
    p += 46 + nlen + xlen + clen;
  }
  return out;
}
function zipText(entry) {      /* -> Promise<string> ('' if the part is absent) */
  if (!entry) return Promise.resolve('');
  if (entry.method === 0) return Promise.resolve(new TextDecoder().decode(entry.data));
  if (entry.method !== 8 || typeof DecompressionStream === 'undefined') {
    return Promise.reject(new Error('this browser cannot read compressed .xlsx parts'));
  }
...
function colIndex(ref) {         /* "BC12" -> 54 (0-based) */
  var n = 0, m = /^[A-Z]+/.exec(ref);
  if (!m) return 0;
  for (var i = 0; i < m[0].length; i++) n = n * 26 + (m[0].charCodeAt(i) - 64);
  return n - 1;
}
function parseXlsxSheet(xml, shared) {
  var doc = xmlDoc(xml), rows = [], rowEls = doc.getElementsByTagName('row');
  for (var r = 0; r < rowEls.length; r++) {
    var rn = parseInt(rowEls[r].getAttribute('r'), 10) || (rows.length + 1);
    while (rows.length < rn - 1) rows.push([]);
    var cells = rowEls[r].getElementsByTagName('c'), vals = [];
    for (var c = 0; c < cells.length; c++) {
      var ce = cells[c], ci = colIndex(ce.getAttribute('r') || ''), type = ce.getAttribute('t') || '';
      var v = '', vEl = ce.getElementsByTagName('v')[0];
      if (type === 'inlineStr') {
        var ts = ce.getElementsByTagName('t');
        for (var k = 0; k < ts.length; k++) v += ts[k].textContent;
      } else if (type === 's') {
        v = shared[parseInt(vEl ? vEl.textContent : '-1', 10)] || '';
      } else if (type === 'b') {
        v = (vEl && vEl.textContent === '1') ? 'TRUE' : 'FALSE';
      } else {
        v = vEl ? vEl.textContent : '';   /* numbers (incl. date serials) + formula strings */
      }
      while (vals.length < ci) vals.push('');
      vals[ci] = v;
    }
    rows.push(vals);
  }
...
function parseXlsx(buf) {        /* -> Promise<{ sheetName: rows }> */
  var z = zipEntries(buf);
  return Promise.all([zipText(z['xl/workbook.xml']), zipText(z['xl/_rels/workbook.xml.rels']), zipText(z['xl/sharedStrings.xml'])])
    .then(function (parts) {
      var wb = xmlDoc(parts[0]), rels = xmlDoc(parts[1]), shared = [];
      if (parts[2]) {
        var sis = xmlDoc(parts[2]).getElementsByTagName('si');
      ...
      var target = {}, rs = rels.getElementsByTagName('Relationship');
      for (var r = 0; r < rs.length; r++) {
        var t = rs[r].getAttribute('Target') || '';
        target[rs[r].getAttribute('Id')] = t.charAt(0) === '/' ? t.slice(1) : 'xl/' + t;
      }
      var sheets = wb.getElementsByTagName('sheet'), jobs = [];
      for (var s = 0; s < sheets.length; s++) {
        (function (el) {
          var name = el.getAttribute('name');
          var rid = el.getAttribute('r:id') || el.getAttributeNS('http://schemas.openxmlformats.org/officeDocument/2006/relationships', 'id');
          jobs.push(zipText(z[target[rid]]).then(function (xml) { return { name: name, rows: parseXlsxSheet(xml, shared) }; }));
        })(sheets[s]);
      }
      return Promise.all(jobs);
    })
    .then(function (list) {
      var out = {};
      list.forEach(function (s) { out[s.name] = s.rows; });
      return out;
    });
}

/* Read the chosen file: a native .xlsx (zip signature "PK") goes through the
   xlsx reader; anything else is treated as a legacy SpreadsheetML export. */
function importFromExcel(file) {
  var reader = new FileReader();
  reader.onload = function () {
    var buf = reader.result, head = new Uint8Array(buf);
    var isZip = head.length > 1 && head[0] === 0x50 && head[1] === 0x4B;
    var p = isZip
      ? parseXlsx(buf)
      : new Promise(function (resolve) { resolve(parseWorkbook(new TextDecoder().decode(buf))); });
    p.then(processImport).catch(function (e) { showToast('Could not read the file: ' + e.message, { error: true }); });
  };
  reader.onerror = function () { showToast('Could not read the file.', { error: true }); };
...
function processImport(book) {
  ...
  Promise.all(pending).then(function () {
    ...
    showConfirm({
      ...
      message: summary,
      confirmLabel: 'Import',
      onConfirm: function () { executeImport(ops, warnings); }
    });
  });
}
