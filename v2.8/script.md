...
/* Selectable Status values when EDITING a Risk — the inline Status dropdown on a
   risk row offers only these. Distinct from RISK_STATES, which controls which
   risks are DISPLAYED. "Closed" isn't present in the loaded data, so its path is
   derived from the environment's own State prefix (e.g. /CaseState/Closed). */
var RISK_STATE_OPTIONS = ['Open', 'Closed'];

/* Selectable states when editing an ActionItem; new actions default to Open.
   This tenant's ActionItemState enum is Open / Completed / Cancelled /
   In-Progress / Rejected — it has NO "Closed", so the done state is Completed.
   Only these two are offered (current value kept if it's another state). */
var ACTION_STATE_OPTIONS = ['Open', 'Completed'];
...
/* Styled in-panel confirmation dialog (NOT the browser's window.confirm).
   opts = { title, message, confirmLabel, danger?, onConfirm }. Enter confirms,
   Esc / backdrop-click / Cancel dismisses. */
function showConfirm(opts) {
  opts = opts || {};
  var back = document.createElement('div');
  back.className = 'aw-confirm-back';
  back.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(32,33,36,0.4);' +
    'z-index:2000;display:flex;align-items:center;justify-content:center;';
  var card = document.createElement('div');
  card.style.cssText = 'background:#fff;max-width:380px;width:88%;border-radius:8px;' +
    'box-shadow:0 10px 34px rgba(0,0,0,0.28);padding:18px 20px;' +
    'font:14px/1.5 system-ui,-apple-system,"Segoe UI",Roboto,sans-serif;color:#202124;';
  var okBg = opts.danger ? '#A32D2D' : '#8B3A62';
  card.innerHTML =
    '<div style="font-weight:600;font-size:15px;margin-bottom:8px;">' + esc(opts.title || 'Confirm') + '</div>' +
    '<div style="margin-bottom:18px;color:#3c4043;">' + esc(opts.message || '') + '</div>' +
    '<div style="display:flex;justify-content:flex-end;gap:8px;">' +
      '<button class="aw-c-cancel" style="padding:7px 14px;border:1px solid #dadce0;background:#fff;color:#3c4043;border-radius:4px;cursor:pointer;font:inherit;">Cancel</button>' +
      '<button class="aw-c-ok" style="padding:7px 14px;border:none;background:' + okBg + ';color:#fff;border-radius:4px;cursor:pointer;font:inherit;">' + esc(opts.confirmLabel || 'Confirm') + '</button>' +
    '</div>';
  ...
  card.querySelector('.aw-c-cancel').addEventListener('click', close);
  card.querySelector('.aw-c-ok').addEventListener('click', go);
  document.addEventListener('keydown', onKey, true);
  (document.querySelector('.aw-panel') || document.body).appendChild(back);
  var okBtn = card.querySelector('.aw-c-ok'); if (okBtn) okBtn.focus();
}
...
function confirmCreateIssue(cell) {
  if (!cell) return;
  var name = cell.getAttribute('data-name') || 'this risk';
  showConfirm({
    title: 'Create Issue from Risk',
    message: 'Escalate "' + name + '" to an Issue? The risk will be set to Realised, a linked Issue created, and the risk\'s actions marked Completed.',
...
function startEdit(cell) {
  ...
  if (isPick(field)) {
    /* A Risk's Status offers only Open / Closed; other State cells (Issues,
    ...
    var optsHtml = (field === 'State' && cell.closest && cell.closest('#risks-body'))
      ? riskStateOptionsHtml(value, false)
      : pickOptionsHtml(field, value, false);
...
var SORT_COLS = {
  risks:  { 2: { f: 'name', t: 's' }, 5: { f: 'riskRating', t: 'n' }, 6: { f: 'status', t: 's' }, 7: { f: 'owner', t: 's' }, 9: { f: 'impactDate', t: 'd' } },
  issues: { 2: { f: 'name', t: 's' }, 4: { f: 'score',      t: 'n' }, 5: { f: 'status', t: 's' }, 6: { f: 'owner', t: 's' }, 8: { f: 'impactDate', t: 'd' } }
};
...
/* Show a ▲/▼ marker on the active sort column (base label cached on the th). */
function updateSortIndicators(tab) {
  ...
  for (var i = 0; i < ths.length; i++) {
    ...
    if (!cols[i]) continue;
    th.textContent = (st.idx === i) ? base + (st.dir > 0 ? ' ▲' : ' ▼') : base;
  }
}
...
function initUI() {
  ...
  ['risks-body', 'issues-body', 'requests-body'].forEach(function (id) {
    ...
    tbody.addEventListener('click', function (e) {
      ...
      if (ci) { confirmCreateIssue(ci.closest('.row-action-cell')); return; }
...
function loadAndRender() {
  ...
  var qIssues =
    "SELECT SYSID, Title, C_IssueImpact, C_IssueScore, State, Owner.Name, C_ReportingLevel, DueDate" +
