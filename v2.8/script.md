...
/* Impact Date field API name on the ISSUE entity. In this tenant the Issue
   entity has NO C_ImpactDate field (create/update reject it with InvalidField),
   so it's disabled ('') for Issue: the Issues "Impact Date" column shows
   read-only "—" and it's omitted from the issue query / create / edit / import.
   Risk keeps C_ImpactDate. If Issue has an impact-date field under another name,
   set this to its API name to re-enable the column for Issues. */
...
function newRowCellHtml(cell) {
  ...
  if (cell.type === 'date') {
    ...
    var input = cell.field
      ? '<input type="date" class="new-input edit-input" data-field="' + cell.field + '" />'
      : '';
